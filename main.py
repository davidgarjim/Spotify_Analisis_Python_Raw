...
def eda(data):   #EDA
    ...
    null_counts.to_csv('output/df_nulos.csv')
    print(null_counts)
...
def var_num(data):
    ...
    plt.tight_layout()
    plt.savefig('output/histograms_variables.png')
    plt.show()
    ...
    plt.savefig('output/correlacion_heatmap.png')
    plt.show()
    ...
    for x, y in scatter_features:
        ...
        plt.savefig(f'output/scatterplot_{x.lower()}_{y.lower()}.png')
        plt.show()
...
def genre(data):   #GÉNEROS
    ...
    print('\n\n', df_recuento_generos)
    df_recuento_generos.to_csv('output/df_recuento_generos.csv')
...
def favs_vs_2023_vs_history(data):     #COMPARACIÓN CON TOP ESPAÑA 2023 Y MOST STREAMED SONGS IN HISTORY
    ...
    for var in ['Popularity', 'Duration'] + vars:
        for plot_type in ['violin', 'box']:
            ...
            plt.ylabel(var)
            plt.savefig(f'output/distribucion_{var.lower()}_{plot_type}.png')
            plt.close()
    ...
    for var in vars:
        ...
        for values, metric_name, plot_title in zip([means, medians], ['Media', 'Mediana'],
            ...
            plt.savefig(f'output/{metric_name.lower()}_{var.lower()}.png')
            plt.close()
...
def artistas_albums(data):  #ARTISTAS Y ÁLBUMS
    ...
    df_recuento_albums.to_csv('output/df_recuento_artistas.csv')
...
def playlist(data): #MIS PLAYLIST POR VARIABLES: MODELO DE CLUSTERIZACIÓN
    ...
    plt.gca().spines[['top', 'right']].set_visible(False)
    plt.savefig('output/scatterplot_energy_acousticness_kmeans.png')
    plt.show()
    ...
    plt.gca().spines[['top', 'right']].set_visible(False)
    plt.savefig('output/scatterplot_energy_acousticness_dbscan.png')
    plt.show()
